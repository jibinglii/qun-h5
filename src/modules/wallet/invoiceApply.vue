<template>
  <div class>
    ...
    <van-cell-group title="申请信息">
      <van-field label="开票金额" input-align="right" v-model="invoice.amount" placeholder="请输入开票金额" />
    </van-cell-group>
    <div class="btn">
      <van-button type="primary" hairline size="large" @click="next()">提交申请</van-button>
    </div>
  </div>
</template>
  ...
  methods: {
    getBillInfo() {
      this.$http.get("api/v2/alliance/bill/apply").then(data => {
		this.billInfo.bill = data.data.bill;
      });
    },
    next() {
      //提交开票信息
      ...
      this.$toast.loading();
      this.invoice.bill_id = this.billInfo.bill.id;
      this.$http
        .post("api/v2/alliance/billing", this.invoice)
        .then(data => {
          if (data.code == 200) {
